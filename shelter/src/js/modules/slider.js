...
const renderSlides = (cardsPerSlide) => {
  ...
  const fragment = getCardsFragment(pets, cardsPerSlide);
  cleanDOM(slides);
...
const drawCards = () => {
  if (slides) {
    if (window.innerWidth >= 755 && window.innerWidth <= 1140) {
      renderSlides(2);
  ...
  if (e.target.closest('.right-arrow')) {
    ...
    setTimeout(() => {
      ...
      if (cardsPerSlide > 1) {
        ...
        const fragment = getCardsFragment(shuffledCards, cardsPerSlide).children[0];
        slides.append(fragment);
      } else {
